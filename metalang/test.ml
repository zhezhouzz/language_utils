...
type 't raw_term =
  ...
  | Ite of ('t, 't raw_term) typed * ('t, 't raw_term) typed * ('t,
  't raw_term) typed 
...
and 't raw_match_case =
  ...
  exp: ('t, 't raw_term) typed } [@@deriving sexp]
let rec fv_raw_term (raw_term_e : 't raw_term) =
  ...
  | Lam { lamarg; lambody } ->
      Zzdatatype.Datatype.List.substract (typed_eq String.equal)
        ([] @ (typed_fv_raw_term lambody)) [lamarg]
  | Err -> []
  | Let { rhs; lhs; letbody;_} ->
      (Zzdatatype.Datatype.List.substract (typed_eq String.equal)
         ([] @ (typed_fv_raw_term letbody)) lhs)
...
and fv_raw_match_case (raw_match_case_e : 't raw_match_case) =
  ...
  | Matchcase { args; exp;_} ->
      Zzdatatype.Datatype.List.substract (typed_eq String.equal)
        ([] @ (typed_fv_raw_term exp)) args
and typed_fv_raw_match_case
...
let rec subst_raw_term (string_x : string) f (raw_term_e : 't raw_term) =
  ...
  | Lam { lamarg; lambody } ->
      if String.equal lamarg.x string_x
      then Lam { lamarg; lambody }
      else
        Lam { lamarg; lambody = (typed_subst_raw_term string_x f lambody) }
  | Err -> Err
  | Let { if_rec; rhs; lhs; letbody } ->
      if List.exists (fun x -> String.equal string_x x.x) lhs
      then
        Let
          { if_rec; rhs = (typed_subst_raw_term string_x f rhs); lhs; letbody
          }
      else
        Let
          {
...
and subst_raw_match_case (string_x : string) f
  ...
  | Matchcase { constructor; args; exp } ->
      if List.exists (fun x -> String.equal string_x x.x) args
      then Matchcase { constructor; args; exp }
      else
        Matchcase
...
let rec map_raw_term (f : 't -> 's) (raw_term_e : 't raw_term) =
  ...
  | Lam { lamarg; lambody } ->
      Lam
        { lamarg = (lamarg #=> f); lambody = (typed_map_raw_term f lambody) }
  | Err -> Err
  | Let { if_rec; rhs; lhs; letbody } ->
      Let
        {
...
let typed_fv_raw_match_case_id e =
  fv_typed_id_to_id typed_fv_raw_match_case e
